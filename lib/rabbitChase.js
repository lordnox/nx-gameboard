...
exports.RabbitChaseBoard = ({ seed, cols = 6, rows = 6, maxItems = Math.floor((cols / 2) + (rows / 2) + 1), } = {}) => {
    ...
    return {
        ...
        drawBoard: (showHidden = false) => {
            ...
            const localBoard = gameboard_1.generateGameboard({
            ...
            return gameboard_1.drawBoard(localBoard, showHidden);
        },
        getInitialSolution: () => {
            ...
            return solution_1.Solution(board, cell => hidden.includes(cell.state), unknown, cell => cell.state, sumHints, actions, (cells, action, payload) => cells);
        },
        toJSON,
    };
};

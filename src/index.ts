...
export interface Gameboard<Cell extends BaseCell = BaseCell> {
  rows: CellGetter<Cell>[][]
  cols: CellGetter<Cell>[][]
...
// flattens an array
export const arrayConcat = <T>(array: T[][]): T[] => [].concat.apply([], array)
...
export const drawBoard = <T>(board: Gameboard, options?: T) => {
  ...
  const content = board.rows.map(row => {
    const content = row.map(cellGetter => {
      ...
      return cell.draw.call(cell, options)
    }).join(' | ')
    return `┃ ${content} ┃`
  }).join(`\n┠─${center}─┨\n`)
  return `┏━${header}━┓\n${content}\n┗━${footer}━┛`
...
export const generateGameboard = <Cell extends BaseCell = BaseCell>(
  ...
  const rows: Getter[][] = range(settings.rows).map(() => [])
  const cols: Getter[][] = range(settings.cols).map(() => [])
